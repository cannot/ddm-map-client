    ...
    <div class="flex h-full z-[1]">
      <div class="w-1/5 md:w-1/5 m-3">
        <div class="relative flex-1 md:min-w-[350px]">
          ...
          <label for="areaCodeGroup">กลุ่มพื้นที่เฝ้าระวัง:</label>

          <select name="areaCodeGroup" id="areaCodeGroup"
            class="pl-9 pr-4 py-3 text-[14px] focus:outline-none w-full shadow-md rounded-md my-2">
            <option value="">no</option>
          </select>

          <label for="areaCode">พื้นที่เฝ้าระวัง:</label>
      ...
      queryTimeout.value = setTimeout(async () => {
        ...
        if (searchQuery.value !== "") {
          const params = new URLSearchParams({
            AREA_GROUP_CODE: areaCodeGroup.value,
            AREA_CODE: areaCode.value,
            proximity: props.coords ? `${props.coords.lng},${props.coords.lat}` : "0,0",
          });
          const data = await axios.get(`http://localhost:3000/api/search/p510/p510?${params}`);
